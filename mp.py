...
def approve_user(message):
    ...
    if user_id in admin_ids or user_id == owner_id:
        ...
        if len(command) == 3:
            ...
            response = f"User ID: {user_to_approve} approved for {duration} 👍."
        else:
            ...
            if user_to_remove in allowed_user_ids:
                ...
                with open(USER_FILE, "w") as file:
                    for user in allowed_user_ids:
                        file.write(f"{user}\n")
                response = f"User ID: {user_to_remove} removed successfully from user list 👍."
            else:
...
def add_admin(message):
    ...
    if user_id == owner_id:
        ...
        if len(command) == 3:
            ...
            response = f"User ID: {admin_to_add} admin approved with balance {balance} 👍."
        else:
    ...
    if user_id == owner_id:
        ...
        if len(command) == 2:
            ...
            if admin_to_remove in admin_ids:
                ...
                response = f"User ID: {admin_to_remove} removed successfully from admin list 👍."
            else:
...
def redeem_gift(message):
    ...
    if len(command) == 2:
        ...
        if code in gift_codes:
            ...
            response = f"🎁 Gift code redeemed ✅\n☑️ You have been granted access for {duration}\n👍 You can now use the bot."
        else:
            response = "Invalid or expired or already used code ❌."
    else:
        response = "⚠️ Usage: /redeem <code>"
...
def cooldown_removal(user_id):
    ...
    response = "*⚡ Attack initiated... Please wait while we process your request!*"
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"*✅ Attack started successfully!*\n*🎯 Target* : `{target}`\n*🌀 Port* : `{port}`\n*⏱️ Duration* : `{time}` seconds\n*⚙️ Method* : Black-Hat Antiban Injected\n*📊 Api response* : `''status'' : ''success'', ''message'' : ''attack sent successfully''`"
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 300:
                response = "⚠️ Error: Time Interval Must be less then 300 seconds."
            else:
                ...
                response = f"⚠️ User Alert from info ⚠️\n\n✅ Attack Complete! ✅\n🎯 Target IP: {target}\n🔌 Target Port: {port}\n⏰ Duration: {time}\n🆔 Attack ID: CJ01GW2GF52JJWGk\n\n👤 Username: {user_name}"
        else:
            response = "⚠️ 𝙄𝙉𝙑𝘼𝙇𝙄𝘿 𝙁𝙊𝙍𝙈𝘼𝙏 ⚠️\n\n✅ 𝙋𝙇𝙀𝘼𝙎𝙀 𝙐𝙎𝙀: /attack <𝙏𝙖𝙧𝙜𝙚𝙩> <𝙋𝙤𝙧𝙩> <𝘿𝙪𝙧𝙖𝙩𝙞𝙤𝙣>\n⚔️ 𝙡𝙖𝙪𝙣𝙘𝙝 𝙖 𝘽𝙜𝙢𝙞 𝙖𝙩𝙩𝙖𝙘𝙠 𝙤𝙣 𝙖 𝙩𝙖𝙧𝙜𝙚𝙩 𝙄𝙋 𝙖𝙣𝙙 𝙥𝙤𝙧𝙩"  # Updated command syntax
    else:
        response = "⛔ 𝘼𝙘𝙘𝙚𝙨𝙨 𝙙𝙚𝙣𝙞𝙚𝙙!\n❌ 𝙔𝙤𝙪 𝙖𝙧𝙚 𝙣𝙤𝙩 𝙖𝙪𝙩𝙝𝙤𝙧𝙞𝙯𝙚𝙙 𝙩𝙤 𝙪𝙨𝙚 𝙩𝙝𝙞𝙨 𝙗𝙤𝙩!\n⚠️ 𝙠𝙞𝙣𝙙𝙡𝙮 𝘿𝙢 𝙏𝙤 𝙂𝙚𝙩 𝘼𝙘𝙘𝙚𝙨𝙨:\n✈️ 𝘾𝙤𝙣𝙩𝙖𝙘𝙩: @BGMIOFFICIALSERVICE_BOT"
...
def show_help(message):
    help_text ='''😍Welcome to , Bgmi-D-DoS BoT

🤖 Available commands:
💥 /attack : Method For Bgmi Servers. 
💥 /rules : Please Check Before Use !!.
💥 /mylogs : To Check Your Recents Attacks.
💥 /Checkinfo : check Bot Net Channel
💥 /ResellerShip : Contact Owner To The Approval
💥 /redeem : redeem a gift code
💥 /CheckBalance : check balance

🤖 To See Admin Commands:
💥 /admincmd : Shows All Admin Commands.
🤖 To See Owner Command.
💥 /ownercmd : Shows All Owner Commands.
...
def send_welcome(message):
    welcome_message = (
        "✈️ 𝙒𝙚𝙡𝙘𝙤𝙢𝙚 𝙩𝙤 𝘽𝙜𝙢𝙞 𝘼𝙩𝙩𝙖𝙘𝙠 𝘽𝙤𝙩! 🚀\n💖 𝙏𝙝𝙞𝙨 𝙗𝙤𝙩 𝙖𝙡𝙡𝙤𝙬𝙨 𝙮𝙤𝙪 𝙩𝙤 𝙡𝙖𝙪𝙣𝙘𝙝 𝙖 𝘽𝙜𝙢𝙞 𝙖𝙩𝙩𝙖𝙘𝙠 𝙤𝙣 𝙖 𝙩𝙖𝙧𝙜𝙚𝙩 𝙄𝙋 𝙖𝙣𝙙 𝙥𝙤𝙧𝙩\n🔥 𝘾𝙤𝙢𝙢𝙖𝙣𝙙 *: KEY BOARD*\n\n")
    ...
    markup.row(btn_reseller, btn_help)
    btn_check = types.KeyboardButton('/Checkinfo')
    btn_balance = types.KeyboardButton('/CheckBalance')
    btn_ping = types.KeyboardButton('/BotPing')
    markup.row(btn_check, btn_balance, btn_ping)
